...
export async function trustSignals() {
  const [liveEscrows, completed, events] = await Promise.all([
    prisma.order.count({
      where: { isSeedDemo: false, escrowContractOrderId: { not: null }, status: { in: ["ESCROWED", "SHIPPED", "DELIVERED", "DISPUTED"] } },
    }),
    prisma.order.findMany({
      where: { isSeedDemo: false, status: "COMPLETED" },
      select: { payoutAmount: true, payoutChainId: true, payoutToken: true, dispute: { select: { buyerShareBps: true } } },
    }),
  ...
  for (const o of completed) {
    ...
    const sellerShare = o.dispute?.buyerShareBps != null ? (amount * BigInt(10_000 - o.dispute.buyerShareBps)) / 10_000n : amount;
    settled += toUsdMicros(sellerShare, o.payoutChainId, o.payoutToken);
  }
  return { liveEscrows, volumeSettledUsdMicros: settled, completedOrders: completed.length, chainEvents: events };
}
...
async function chainHealth() {
  return Promise.all(
    chainProfiles().map(async (p) => {
      ...
      const base = { chainId: p.chain.id, name: p.label, role: p.role, explorerUrl: p.explorerUrl ?? null, deployed: !!dep };
      if (!dep) return { ...base, online: false, blockNumber: null, liquidity: [], paymasterDeposit: null, contracts: null };
      const client = publicClient(p.chain.id);
      try {
        const [blockNumber, usdcLiq, daiLiq, pmDeposit, checkpoint] = await Promise.all([
          client.getBlockNumber(),
          client.readContract({ address: dep.paymentRouter, abi: trestlePaymentRouterAbi, functionName: "totalLiquidity", args: [dep.usdc] }),
          client.readContract({ address: dep.paymentRouter, abi: trestlePaymentRouterAbi, functionName: "totalLiquidity", args: [dep.dai] }),
          client.readContract({ address: dep.entryPoint, abi: entryPointAbi, functionName: "balanceOf", args: [dep.paymaster] }),
          prisma.relayerCheckpoint.findUnique({ where: { id: String(p.chain.id) } }),
        ]);
        const escrowUsdc = await client.readContract({ address: dep.usdc, abi: erc20Abi, functionName: "balanceOf", args: [dep.escrow as Address] });
        const escrowDai = await client.readContract({ address: dep.dai, abi: erc20Abi, functionName: "balanceOf", args: [dep.escrow as Address] });
        return {
          ...base,
          online: true,
        ...
        return { ...base, online: false, blockNumber: null, liquidity: [], paymasterDeposit: null, contracts: null };
      }
    }),
  );
}
...
export async function protocolStats() {
  const [signals, intentGroups, disputes, recentIntents, recentEvents, seedDemoOrders, orderGroups, gasEvents, createdEvents, fulfilledEvents, chains] =
    await Promise.all([
      trustSignals(),
      prisma.paymentIntent.groupBy({ by: ["status", "routeKind"], _count: { _all: true }, where: { order: { isSeedDemo: false } } }),
      prisma.dispute.findMany({ where: { order: { isSeedDemo: false } }, select: { status: true, buyerShareBps: true } }),
      prisma.paymentIntent.findMany({
        where: { order: { isSeedDemo: false }, OR: [{ sourceTxHash: { not: null } }, { status: { not: "CREATED" } }] },
        orderBy: { updatedAt: "desc" },
        take: 25,
        include: { order: { select: { id: true, status: true, escrowContractOrderId: true, escrowChainId: true, subtotalUsdMicros: true } } },
      }),
      prisma.chainEvent.findMany({ orderBy: [{ blockTime: "desc" }, { createdAt: "desc" }], take: 40 }),
      prisma.order.count({ where: { isSeedDemo: true } }),
      prisma.order.groupBy({ by: ["status"], _count: { _all: true }, where: { isSeedDemo: false } }),
      prisma.chainEvent.findMany({ where: { eventName: "GasSponsored" }, select: { args: true, chainId: true } }),
      prisma.chainEvent.findMany({ where: { eventName: "IntentCreated" }, select: { args: true, blockTime: true } }),
      prisma.chainEvent.findMany({ where: { eventName: "IntentFulfilled" }, select: { args: true, blockTime: true } }),
      chainHealth(),
    ]);

  const createdAt = new Map(createdEvents.map((e) => [String((e.args as Record<string, unknown>).intentId).toLowerCase(), e.blockTime]));
  ...
  const gasSponsoredWei = gasEvents.reduce((s, e) => s + BigInt(String((e.args as Record<string, unknown>).actualGasCost ?? 0)), 0n);
  ...
  return {
    ...
    intents: intentGroups.map((g) => ({ status: g.status, routeKind: g.routeKind, count: g._count._all })),
    settlement: {
