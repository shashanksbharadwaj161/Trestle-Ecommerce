...
export async function smartAccountFor(userId: string, owner: Address, chainId: number): Promise<Address> {
  const cached = await prisma.smartAccount.findUnique({ where: { userId_chainId: { userId, chainId } } });
  const dep = requireDeployment(chainId);
  if (cached && cached.factory === dep.accountFactory.toLowerCase()) return getAddress(cached.address);
  ...
  await prisma.smartAccount.upsert({
    ...
    create: { userId, chainId, address: address.toLowerCase(), factory: dep.accountFactory.toLowerCase() },
...
export async function addressesOf(userId: string): Promise<string[]> {
  const user = await prisma.user.findUnique({ where: { id: userId }, include: { smartAccounts: true } });
  if (!user) return [];
