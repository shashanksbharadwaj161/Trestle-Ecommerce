...
export function buildChains(cfg: RelayerConfig): Map<number, ChainCtx> {
  ...
  for (const profile of cfg.profiles) {
    ...
    if (!dep) throw new Error(`No ${cfg.mode} deployment for chain ${profile.chain.id} — deploy contracts first`);
    ...
    map.set(profile.chain.id, {
      ...
      dep,
      client: createPublicClient({ chain: profile.chain, transport, pollingInterval }) as PublicClient,
