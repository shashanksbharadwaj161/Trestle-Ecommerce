...
export function ProductAdminList({
  ...
  const del = useMutation({
    mutationFn: (id: string) => api<{ archived: boolean; reason?: string }>(`/api/products/${id}`, { method: "DELETE" }),
    onSuccess: (r) => {
  ...
  return (
    <div>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        >
          <Input aria-label="Search products" placeholder="Search products" value={search} onChange={(e) => setSearch(e.target.value)} className="h-10 w-64" />
          ...
          <table className="w-full min-w-[760px] text-sm">
            ...
            <thead className="bg-muted/50 text-left text-xs text-muted-foreground">
              <tr>
                ...
                <th scope="col" className="p-3 font-medium">Colours</th>
            ...
            <tbody>
              {q.data!.items.map((p) => {
                ...
                const soldOutSkus = p.sizes.flatMap((s) => s.variants).filter((v) => v.stock <= 0).length;
                return (
                  <tr key={p.id} className="border-t border-border">
                    <td className="p-3">
                      <div className="flex items-center gap-3">
                        <div className="relative aspect-[3/4] w-10 shrink-0 bg-muted">
                          {p.colours[0]?.image?.startsWith("/") && <Image src={p.colours[0].image} alt="" fill sizes="40px" className="object-cover" />}
                        </div>
                        <div>
                          <Link href={`${basePath}/${p.id}`} className="font-medium hover:underline">
                            {p.title}
                          </Link>
                          <p className="text-xs text-muted-foreground">{CATEGORY_LABEL[p.category as Category] ?? p.category}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-3"><Price micros={p.priceUsdMicros} /></td>
                    <td className="p-3">
                      <div className="flex gap-1">
                        {p.colours.map((c) => (
                          <span key={c.name} title={c.name} className="size-4 rounded-full ring-1 ring-foreground/15" style={{ background: c.hex ?? "var(--muted)" }} />
                        ))}
                      </div>
                    </td>
                    <td className="tabular p-3">
                      {stock}
                      {soldOutSkus > 0 && <span className="ml-2 text-xs text-muted-foreground">({soldOutSkus} SKU sold out)</span>}
                    </td>
                    <td className="p-3">
                      <Badge tone={p.status === "ACTIVE" ? "success" : p.status === "DRAFT" ? "warning" : "neutral"}>{(p.status ?? "").toLowerCase()}</Badge>
                    </td>
                    <td className="p-3">
                      <div className="flex justify-end gap-1">
                        {extraActions?.(p)}
                        <Button asChild size="icon" variant="ghost" aria-label={`Edit ${p.title}`}>
                          <Link href={`${basePath}/${p.id}`}><Pencil /></Link>
                        </Button>
                        <Button
                          size="icon"
                          variant="ghost"
                          aria-label={`Delete ${p.title}`}
                          onClick={() => confirm(`Delete “${p.title}”? Products with orders are archived instead.`) && del.mutate(p.id)}
                        >
                          <Trash2 />
                        </Button>
                      </div>
