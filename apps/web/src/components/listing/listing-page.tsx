...
export async function ListingPage({
  ...
  const density: Density =
    (await cookies()).get(DENSITY_COOKIE)?.value === "large" ? "large" : "standard";
  const cardSizes =
    density === "large"
  ...
  items.forEach((p, i) => {
    ...
    if (showEditorial && (i + 1) % 8 === 6 && preset.editorial![e])
      cells.push({ kind: "e", e: preset.editorial![e++]! });
  });
      ...
      <div className="container-page pt-6 md:pt-8">
        ...
        <div className="mt-6 flex flex-col gap-2 md:mt-10">
          <h1 className="text-[2rem] leading-[1.05] tracking-[-0.03em] md:text-[2.75rem]">
            {q.q && !preset.department && !preset.collection ? (
              <>Results for “{q.q}”</>
            ) : (
              preset.title
            )}
          ...
          {preset.description && (
            <p className="max-w-xl text-sm text-muted-foreground">{preset.description}</p>
          )}
        </div>
        {preset.chips && preset.chips.length > 0 && (
          <nav
            aria-label="Categories"
            className="no-scrollbar -mx-4 mt-6 flex gap-2 overflow-x-auto px-4 md:mx-0 md:px-0"
          >
            ...
            {preset.chips.map((c) => (
              <Chip
                key={c}
                href={`${preset.basePath}?category=${c}`}
                active={q.category?.length === 1 && q.category[0] === c}
              >
                {CATEGORY_LABEL[c]}
              </Chip>
            ))}
        ...
        <div className="container-page mt-12 flex flex-col items-center gap-4">
          ...
          <div className="h-[2px] w-40 bg-border" aria-hidden>
            <div
              ...
              style={{ width: `${Math.round((items.length / Math.max(1, data.total)) * 100)}%` }}
            />
          </div>
          {items.length < data.total && (
            <Link
              href={nextHref}
              scroll={false}
              className="mt-2 inline-flex h-11 items-center border border-foreground px-8 text-sm transition-colors hover:bg-foreground hover:text-background"
            >
              Load more
            </Link>
          )}
