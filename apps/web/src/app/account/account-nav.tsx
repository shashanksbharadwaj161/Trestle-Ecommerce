...
export function AccountNav() {
  ...
  return (
    <div className="border-b border-border">
      <nav aria-label="Account" className="container-page no-scrollbar flex items-center gap-6 overflow-x-auto">
        {LINKS.map((l) => (
          <Link
            key={l.href}
            href={l.href}
        ...
        {(user.sellerId || user.role === "ADMIN") && (
          <Link href={user.role === "ADMIN" ? "/admin" : "/seller/products"} className="flex h-12 shrink-0 items-center text-[0.8125rem] text-muted-foreground hover:text-foreground">
