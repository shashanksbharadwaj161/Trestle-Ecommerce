...
export default async function ProductPage({
  ...
  if (product.slug && id !== product.slug) {
    permanentRedirect(
      `/products/${product.slug}${sp.colour ? `?colour=${encodeURIComponent(sp.colour)}` : ""}`,
    );
  }

  const [provenance, relatedRows] = await Promise.all([
    provenanceFor(
      product.certificates.map((c) => ({ chainId: c.chainId, contractAddress: c.contractAddress })),
    ),
    relatedProducts({ id: product.id, department: product.department, category: product.category }),
  ]);
  ...
  const related = toJsonSafe(relatedRows) as unknown as CardData[];
      ...
      <div className="container-page pt-4 md:pt-6">
        <Breadcrumb
          items={[
            ...
            {
              label: CATEGORY_LABEL[cat] ?? product.category,
              href: `${deptHref[dept]}?category=${cat}`,
            },
            { label: product.title },
          ]}
        />
      ...
      {product.reviews.length > 0 && (
        <section aria-labelledby="reviews" className="container-page mt-20">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Reviews can only be written after a completed purchase. Average{" "}
            {product.rating.avg.toFixed(1)} / 5.
          </p>
      ...
      {pay.crypto && certs.length > 0 && (
        <section aria-labelledby="provenance" className="container-page mt-20">
          ...
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
            The seller minted an on-chain certificate for this item. It records who issued it and
            every transfer — it is a record kept by the seller, not an independent certification.{" "}
