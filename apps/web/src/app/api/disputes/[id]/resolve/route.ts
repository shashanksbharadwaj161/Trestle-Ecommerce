...
export const POST = route<{ id: string }>(
  ...
  async ({ req, params, user }) => {
    ...
    const dispute = await prisma.dispute.findUnique({ where: { id: params.id }, include: { order: true } });
    if (!dispute) throw notFound("Dispute");
    if (dispute.status !== "OPEN") throw conflict("Dispute is already resolved");
    if (!dispute.raiseTxHash || !dispute.order.escrowChainId || !dispute.order.escrowContractOrderId) {
    ...
    if (!isArbiter) throw forbidden("Your wallet does not hold ARBITER_ROLE on the escrow contract");

    await prisma.dispute.update({ where: { id: dispute.id }, data: { resolutionNotes: input.notes } });
