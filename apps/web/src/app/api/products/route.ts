...
export const GET = route({ rateLimit: { bucket: "products-read", limit: 240, windowSec: 60 } }, async ({ req, user }) => {
...
export const POST = route(
  ...
  async ({ req, user }) => {
    ...
    const taken = await prisma.productVariant.findFirst({ where: { sku: { in: skus } }, select: { sku: true } });
    ...
    const product = await prisma.product.create({
      data: {
        ...
        variants: {
          create: input.variants.map((v) => ({ name: v.name, sku: v.sku, stock: v.stock, attributes: v.attributes })),
        },
      },
      include: { variants: true },
    });
    await prisma.auditLog.create({
      data: { actorId: user!.id, action: "product.create", entity: "Product", entityId: product.id, data: {} },
    });
    return { product };
  },
);
