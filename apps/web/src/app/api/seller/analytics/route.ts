...
export const GET = route({ auth: "seller" }, async ({ user }) => {
  ...
  const orders = await prisma.order.findMany({
    ...
    select: { id: true, status: true, subtotalUsdMicros: true, createdAt: true, isSeedDemo: true, items: { select: { titleSnapshot: true, quantity: true, productId: true } }, dispute: { select: { status: true, buyerShareBps: true } } },
  ...
  const repEvents = await prisma.reputationEvent.findMany({
    ...
    orderBy: { createdAt: "asc" },
    select: { createdAt: true, newScore: true, eventType: true, weight: true, chainId: true, isSeedDemo: true },
  });
  return {
    revenueUsdMicros: paid.reduce((s, o) => s + o.subtotalUsdMicros, 0n),
    paidOrders: paid.length,
    averageOrderUsdMicros: paid.length ? paid.reduce((s, o) => s + o.subtotalUsdMicros, 0n) / BigInt(paid.length) : 0n,
    disputeRate: paid.length ? disputed / paid.length : 0,
    statusCounts,
    daily: [...byDay.values()],
    topProducts: [...products.entries()].map(([id, p]) => ({ id, ...p })).sort((a, b) => b.units - a.units).slice(0, 5),
    reputationTrend: repEvents.map((e) => ({ at: e.createdAt, score: Number(e.newScore), eventType: e.eventType, weight: e.weight, isSeedDemo: e.isSeedDemo })),
