...
export const GET = route({ auth: "user" }, async ({ user }) => {
  const seller = user!.sellerId
    ? await prisma.seller.findUnique({ where: { id: user!.sellerId } })
    : null;
  const options = chainProfiles()
    ...
    .map((p) => ({
      chainId: p.chain.id,
      chainName: p.label,
      tokens: payoutTokens(env().mode, p.chain.id).map((t) => ({
...
export const POST = route(
  { auth: "user", rateLimit: { bucket: "onboarding", limit: 10, windowSec: 60 } },
  async ({ req, user }) => {
    const input = await parseBody(req, onboardingInput);
    const valid = payoutTokens(env().mode, input.payoutChainId).some(
      (t) => t.address.toLowerCase() === input.payoutToken,
    );
    if (!valid)
    ...
    const payoutChanged =
      existing &&
      (existing.payoutAddress !== payoutAddress || existing.payoutChainId !== input.payoutChainId);
    ...
    await prisma.auditLog.create({
      data: {
        actorId: user!.id,
        action: existing ? "seller.update" : "seller.onboard",
        entity: "Seller",
        entityId: seller.id,
        data: {},
      },
