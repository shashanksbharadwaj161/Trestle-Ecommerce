...
export const GET = route({ auth: "user" }, async ({ user }) => {
  ...
  for (const p of chainProfiles()) {
    ...
    const accounts = [];
    for (const [kind, addr] of [["wallet", owner], ["smart", smart]] as const) {
      if (!addr) continue;
      try {
        const a = getAddress(addr);
        const [balance, stake, pending, discount, weight] = await Promise.all([
          client.readContract({ address: dep.loyalty, abi: trestleLoyaltyAbi, functionName: "balanceOf", args: [a] }),
          client.readContract({ address: dep.loyalty, abi: trestleLoyaltyAbi, functionName: "stakeOf", args: [a] }),
          client.readContract({ address: dep.loyalty, abi: trestleLoyaltyAbi, functionName: "pendingRewards", args: [a] }),
          client.readContract({ address: dep.loyalty, abi: trestleLoyaltyAbi, functionName: "feeDiscountBps", args: [a] }),
          client.readContract({ address: dep.loyalty, abi: trestleLoyaltyAbi, functionName: "votingWeight", args: [a] }),
        ]);
    ...
    try {
      const [t, apr] = await Promise.all([
        client.readContract({ address: dep.loyalty, abi: trestleLoyaltyAbi, functionName: "tiers", args: [] }),
        client.readContract({ address: dep.loyalty, abi: trestleLoyaltyAbi, functionName: "rewardAprBps", args: [] }),
      ]);
      tiers = (t as { minStake: bigint; discountBps: number }[]).map((x) => ({ minStake: x.minStake.toString(), discountBps: Number(x.discountBps) }));
      aprBps = Number(apr as bigint);
    } catch {
      /* ignore */
    }
    chains.push({ chainId: p.chain.id, chainName: p.label, loyalty: dep.loyalty, accounts, tiers, rewardAprBps: aprBps, online: accounts.length > 0 });
  }
