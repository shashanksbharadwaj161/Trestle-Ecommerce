...
const post = (path: string, body: unknown, jar = newJar(), headers: Record<string, string> = { origin: ORIGIN }) =>
...
describe("SIWE authentication", () => {
  ...
  it("rejects nonce replay", async () => {
    ...
    const message = createSiweMessage({
      address: account.address, chainId: 31338, domain: "localhost:3000", nonce: n.data.nonce, uri: ORIGIN, version: "1", issuedAt: new Date(),
    });
    const signature = await account.signMessage({ message });
    const first = await call(verifyPOST, post("/api/auth/verify", { message, signature }, jar), undefined, jar);
    ...
    const second = await call(verifyPOST, post("/api/auth/verify", { message, signature }, replayJar));
  ...
  it("rejects a message for another domain and a forged signature", async () => {
    ...
    for (const [domain, signer] of [["evil.example", account], ["localhost:3000", other]] as const) {
      const jar = newJar();
      const n = await call(nonceGET, req("/api/auth/nonce", { jar }), undefined, jar);
      const message = createSiweMessage({
        address: account.address, chainId: 31338, domain, nonce: n.data.nonce, uri: `http://${domain}`, version: "1", issuedAt: new Date(),
      });
...
describe("authorization, validation, CSRF and rate limits", () => {
  ...
  it("lets an onboarded seller create, update and delete products with exact prices", async () => {
    ...
    const patched = await call(
      productPATCH,
      req(`/api/products/${id}`, { method: "PATCH", jar, headers: { origin: ORIGIN }, body: { ...validProduct, price: "59.00" } }),
      { id },
    );
    expect(patched.status).toBe(200);
    expect(patched.data.product.priceUsdMicros).toBe("59000000");
    const del = await call(productDELETE, req(`/api/products/${id}`, { method: "DELETE", jar, headers: { origin: ORIGIN } }), { id });
  ...
  it("forbids editing another seller's product", async () => {
    ...
    const { jar } = await signIn();
    await call(onboardingPOST, post("/api/seller/onboarding", { storefrontName: "Intruder", slug: "intruder", payoutChainId: 31338, payoutToken: "0x5FbDB2315678afecb367f032d93F642f64180aa3" }, jar));
    const res = await call(
      productPATCH,
      req(`/api/products/${product.id}`, { method: "PATCH", jar, headers: { origin: ORIGIN }, body: validProduct }),
      { id: product.id },
    );
  ...
  it("returns field errors for invalid input", async () => {
    const { jar } = await signIn();
    await call(onboardingPOST, post("/api/seller/onboarding", { storefrontName: "Val", slug: "val-co", payoutChainId: 31338, payoutToken: "0x5FbDB2315678afecb367f032d93F642f64180aa3" }, jar));
    const res = await call(productsPOST, post("/api/products", { ...validProduct, price: "12.345", images: ["http://insecure"] }, jar));
    expect(res.status).toBe(400);
    expect(res.data.error.code).toBe("validation_error");
    expect(Object.keys(res.data.error.details.fieldErrors)).toEqual(expect.arrayContaining(["price", "images"]));
  });

  it("rejects cross-origin mutations", async () => {
    const { jar } = await signIn();
    const res = await call(cartPOST, post("/api/cart", { op: "clear" }, jar, { origin: "https://evil.example" }));
    expect(res.status).toBe(403);
  ...
  it("rate limits bursts", async () => {
    ...
    for (let i = 0; i < 12; i++) {
      const r = await call(verifyPOST, post("/api/auth/verify", { message: "x".repeat(60), signature: "0x00" }));
...
describe("catalog & cart", () => {
  it("filters products by search, category, price and chain", async () => {
    await fixtureSeller();
    const r1 = await call(productsGET, req("/api/products?q=watch&category=Watches&maxPrice=200&chain=31337"));
  ...
  it("adds, updates and clamps cart lines to stock", async () => {
    ...
    const added = await call(cartPOST, post("/api/cart", { op: "add", variantId: v, quantity: 5 }, jar));
...
describe("checkout", () => {
  const shipping = { name: "Test Buyer", line1: "1 Test St", city: "Testville", postalCode: "12345", country: "us" };
  ...
  it("quotes exact amounts, reserves stock atomically and is idempotent per quote", async () => {
    ...
    const v = product.variants[0]!.id;
    const { jar } = await signIn();
    const q = await call(quotePOST, post("/api/checkout/quote", { sellerId: seller.id, items: [{ variantId: v, quantity: 1 }] }, jar));
    expect(q.status).toBe(200);
    expect(q.data.subtotalUsdMicros).toBe("125000000");
    const direct = q.data.routes.find((r: any) => r.kind === "direct" && r.payToken.symbol === "tUSDC");
    expect(direct.destAmount).toBe("125000000");
    expect(BigInt(direct.sourceAmount)).toBe(125_000_000n + BigInt(direct.feeAmount));

    const init = await call(initiatePOST, post("/api/checkout/initiate", { quoteId: q.data.quoteId, routeKey: direct.key, buyerAccountMode: "wallet", shippingAddress: shipping }, jar));
    expect(init.status).toBe(200);
    expect(init.data.calls.map((c: any) => c.description)).toEqual(["Approve Trestle router to move your tokens", "Pay into escrow"]);
    ...
    const again = await call(initiatePOST, post("/api/checkout/initiate", { quoteId: q.data.quoteId, routeKey: direct.key, buyerAccountMode: "wallet", shippingAddress: shipping }, jar));
    ...
    const q2 = await call(quotePOST, post("/api/checkout/quote", { sellerId: seller.id, items: [{ variantId: v, quantity: 1 }] }, other.jar));
  ...
  it("prevents concurrent oversell of the last unit", async () => {
    ...
    const qa = await call(quotePOST, post("/api/checkout/quote", { sellerId: seller.id, items: [{ variantId: v, quantity: 1 }] }, a.jar));
    const qb = await call(quotePOST, post("/api/checkout/quote", { sellerId: seller.id, items: [{ variantId: v, quantity: 1 }] }, b.jar));
    const key = (q: any) => q.data.routes.find((r: any) => r.kind === "direct" && r.available).key;
    const [ra, rb] = await Promise.all([
      call(initiatePOST, post("/api/checkout/initiate", { quoteId: qa.data.quoteId, routeKey: key(qa), buyerAccountMode: "wallet", shippingAddress: shipping }, a.jar)),
      call(initiatePOST, post("/api/checkout/initiate", { quoteId: qb.data.quoteId, routeKey: key(qb), buyerAccountMode: "wallet", shippingAddress: shipping }, b.jar)),
    ]);
  ...
  it("rejects quotes owned by another user", async () => {
    ...
    const a = await signIn();
    const b = await signIn();
    const q = await call(quotePOST, post("/api/checkout/quote", { sellerId: seller.id, items: [{ variantId: product.variants[0]!.id, quantity: 1 }] }, a.jar));
    const key = q.data.routes.find((r: any) => r.available).key;
    const res = await call(initiatePOST, post("/api/checkout/initiate", { quoteId: q.data.quoteId, routeKey: key, buyerAccountMode: "wallet", shippingAddress: shipping }, b.jar));
    expect(res.status).toBe(403);
  });
});
  ...
  it("hides orders from non-participants and enforces review/fulfilment rules", async () => {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        items: { create: { productId: product.id, productVariantId: product.variants[0]!.id, titleSnapshot: "Fixture Watch", variantSnapshot: "Black", quantity: 1, unitPriceUsdMicros: 125_000_000n } },
      },
    });
    const stranger = await signIn();
    const hidden = await call(orderGET, req(`/api/orders/${order.id}`, { jar: stranger.jar }), { id: order.id });
    expect(hidden.status).toBe(404);
    const own = await call(orderGET, req(`/api/orders/${order.id}`, { jar: buyer.jar }), { id: order.id });
    expect(own.status).toBe(200);
    expect(own.data.viewer).toBe("buyer");

    const early = await call(reviewPOST, post(`/api/orders/${order.id}/review`, { rating: 5, text: "Great product, fast!" }, buyer.jar), { id: order.id });
    expect(early.status).toBe(409);

    const sellerSession = await signIn(sellerPk);
    const noTracking = await call(fulfillmentPOST, post(`/api/orders/${order.id}/fulfillment`, { action: "ship" }, sellerSession.jar), { id: order.id });
    expect(noTracking.status).toBe(400);
    const shipped = await call(fulfillmentPOST, post(`/api/orders/${order.id}/fulfillment`, { action: "ship", trackingNumber: "1Z999" }, sellerSession.jar), { id: order.id });
    expect(shipped.status).toBe(200);
    const buyerShip = await call(fulfillmentPOST, post(`/api/orders/${order.id}/fulfillment`, { action: "deliver" }, buyer.jar), { id: order.id });
    expect(buyerShip.status).toBe(403);

    await prisma.order.update({ where: { id: order.id }, data: { status: "COMPLETED" } });
    const ok = await call(reviewPOST, post(`/api/orders/${order.id}/review`, { rating: 5, text: "Great product, fast!" }, buyer.jar), { id: order.id });
    expect(ok.status).toBe(200);
    expect(ok.data.review.verifiedPurchase).toBe(true);
    const dup = await call(reviewPOST, post(`/api/orders/${order.id}/review`, { rating: 1, text: "Changing my mind." }, buyer.jar), { id: order.id });
...
describe("chain-events webhook", () => {
  ...
  const send = (body: string, ts: string, sig: string) =>
    call(webhookPOST, req("/api/webhooks/chain-events", { method: "POST", rawBody: body, headers: { "content-type": "application/json", "x-trestle-timestamp": ts, "x-trestle-signature": sig } }));
